def update_data(labelist, goldenAnnotations, transformedList):
    if goldenAnnotations and  labelist and transformedList:
        label_map = {}
        for label_dict in labelist:
            entity_name = label_dict.get("labelName", "")
            color = label_dict.get("color", "")
            label_map[entity_name] = color
        # print(label_map)
        # print("========================================")
        for file_name, annotation_list in goldenAnnotations.items():
            for annotation_dict in annotation_list:
                golden_entity = annotation_dict.get("tagName", "")
                if golden_entity in list(label_map.keys()):
                    update_color = label_map[golden_entity]
                    annotation_dict["color"] = update_color

        for loading_msg in transformedList:
            loading_file_name = loading_msg.get("name", "")
            if loading_file_name in list(goldenAnnotations.keys()):
                loading_msg["entities"] = goldenAnnotations[loading_file_name]
...
#     ]
# }
# update_data(labelist, goldenannot, trans)
